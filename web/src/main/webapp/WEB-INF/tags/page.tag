...
<%@ taglib prefix="es" tagdir="/WEB-INF/tags" %>
    ...
    <c:forEach begin="${end < page.totalPages ? page.totalPages - 1 : page.totalPages + 1}" end="${page.totalPages}" var="i">
      ...
      <c:choose>
        ...
        <c:otherwise>
            <li><a href="#" onclick="$.app.turnPage('${url}', '${pagePrefix}', '${pageSize}', ${current + 1});" title="下一页" async="true">&gt;&gt;</a></li>
            <li><a href="#" onclick="$.app.turnPage('${url}', '${pagePrefix}', '${pageSize}', ${page.totalPages});" title="尾页" async="true">尾页</a></li>
        </c:otherwise>
    </c:choose>
