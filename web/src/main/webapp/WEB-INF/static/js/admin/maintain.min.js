...
if(c.is(":visible")){e();
return}if(!c.length){$.ajax({type:"GET",async:false,url:ctx+"/admin/maintain/icon/select",success:function(f){$("body").append(f)
}});c=$(".sys-icon-list")
}var d=a.offset();
...
}var b=ctx+"/admin/maintain/staticResource/incVersion";
...
d[d.length]=k;
c[c.length]=j
}var b=ctx+"/admin/maintain/staticResource/batchIncVersion";
...
c[c.length]=j;
g[g.length]=f
}var d=ctx+"/admin/maintain/staticResource/clearVersion";
...
var a=ctx+"/admin/maintain/staticResource/compress";
...
c[c.length]=j;
g[g.length]=f
}var d=ctx+"/admin/maintain/staticResource/batchCompress";
$.post(d,{"fileNames[]":c,"contents[]":g},function(i){$.app.alert({message:i,ok:function(){window.location.reload()
}})})}})});
...
var e=d.find("[name='fileName']").val();
var c=d.find("[name='content']").val();
var a=ctx+"/admin/maintain/staticResource/switch";
...
d[d.length]=k;
h[h.length]=g
}var e=ctx+"/admin/maintain/staticResource/batchSwitch";
...
location.href=ctx+"/admin/maintain/editor/rename?path="+c+"&newName="+d
...
return}location.href=ctx+"/admin/maintain/editor/delete?"+a.serialize()
...
}location.href=ctx+"/admin/maintain/editor/create/directory?parentPath="+a+"&name="+b
...
if(!$.trim(b)){$.app.alert({message:"名称不能为空"})
}location.href=ctx+"/admin/maintain/editor/create/file?parentPath="+a+"&name="+b
}})});$(".btn-upload").click(function(){var a=$(this).closest(".tool").data("current-path");
location.href=ctx+"/admin/maintain/editor/upload?parentPath="+a
...
return}window.location.href=ctx+"/admin/maintain/editor/compress?parentPath="+a+"&"+b.serialize()
...
return}$.app.modalDialog("选择解压到的目录",ctx+"/admin/maintain/editor/select?"+b.serialize(),{height:300,width:300,ok:function(f){var e=$.fn.zTree.getZTreeObj(f.find("#selectTree .ztree").attr("id"));
var g=e.getSelectedNodes()[0];
var h=g.path;
window.location.href=ctx+"/admin/maintain/editor/uncompress?descPath="+h+"&conflict="+c+"&"+b.serialize()
...
$.app.modalDialog("选择移动到的目录",ctx+"/admin/maintain/editor/select?"+a.serialize(),{height:300,width:300,ok:function(d){var c=$.fn.zTree.getZTreeObj(d.find("#selectTree .ztree").attr("id"));
var e=c.getSelectedNodes()[0];
var f=e.path;
window.location.href=ctx+"/admin/maintain/editor/move?descPath="+f+"&conflict="+b+"&"+a.serialize()
...
$.app.modalDialog("选择复制到的目录",ctx+"/admin/maintain/editor/select?"+a.serialize(),{height:300,width:300,ok:function(d){var c=$.fn.zTree.getZTreeObj(d.find("#selectTree .ztree").attr("id"));
var e=c.getSelectedNodes()[0];
var f=e.path;
window.location.href=ctx+"/admin/maintain/editor/copy?descPath="+f+"&conflict="+b+"&"+a.serialize()
}})}})})}}};